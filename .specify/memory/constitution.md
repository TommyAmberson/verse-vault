...
Four obligations follow, and each MUST hold independently:

* **Rejections are per item.** One unacceptable item MUST NOT block acceptable ones. A batch
  endpoint accepts what it can and reports what it refused; it MUST NOT fail a whole batch for one
  bad member, because the client's only recourse is to resend the same batch forever.
* **Refusals are machine-readable.** A rejection MUST name what was refused in a field the client
  can act on. Prose is for humans; a client that has to parse an error message to recover is already
  broken.
